-- Get the theme ID for Industrial Energy Efficiency & Optimisation
-- Insert comprehensive detailed scores for all criteria
INSERT INTO detailed_scores (theme_id, criteria_id, score, confidence, notes, analyst_notes, update_source)
SELECT 
  t.id as theme_id,
  fc.id as criteria_id,
  CASE fc.code
    WHEN 'A1' THEN 4  -- Market Attractiveness - Market Size & Growth
    WHEN 'A2' THEN 3  -- Market Attractiveness - Competitive Landscape
    WHEN 'B1' THEN 5  -- Fund Fit - Business Model Alignment
    WHEN 'B2' THEN 4  -- Fund Fit - Strategic Alignment
    WHEN 'C1' THEN 3  -- Investability - Market Fragmentation
    WHEN 'C2' THEN 4  -- Investability - Exit Environment
    WHEN 'D1' THEN 3  -- Risk Profile - Technology Risk
    WHEN 'D2' THEN 2  -- Risk Profile - Regulatory Risk
    WHEN 'E1' THEN 4  -- Right to Win - Team & Network
    ELSE 3
  END as score,
  CASE fc.code
    WHEN 'A1' THEN 'High'
...
INSERT INTO research_documents (theme_id, criteria_id, title, description, document_type, created_by)
SELECT 
  t.id as theme_id,
  fc.id as criteria_id,
  CASE 
    ...
    ELSE 'Supporting Research Documentation'
  END as title,
  CASE 
    ...
    ELSE 'Supporting documentation for investment thesis development'
  END as description,
...
-- Insert regulatory instruments relevant to this theme
