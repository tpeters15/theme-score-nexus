...
-- Link regulations to the Industrial Energy Efficiency theme with relevance scores 1-10 scale
DO $$
DECLARE
    theme_uuid uuid;
BEGIN
    ...
    IF theme_uuid IS NOT NULL THEN
        ...
        SELECT 
          ...
          r.id,
          CASE r.title
            WHEN 'Energy Efficiency Directive (EED) 2023/1791' THEN 9
            WHEN 'Corporate Sustainability Reporting Directive (CSRD)' THEN 8
            WHEN 'EU Taxonomy Regulation' THEN 8
            WHEN 'Sustainable Finance Disclosure Regulation (SFDR)' THEN 7
            WHEN 'EU Emissions Trading System (EU ETS) - Phase 4' THEN 9
            WHEN 'Energy Savings Opportunity Scheme (ESOS)' THEN 7
            WHEN 'Streamlined Energy and Carbon Reporting (SECR)' THEN 6
          END,
          CASE r.title
        ...
        WHERE r.title IN (
          ...
          SELECT 1 FROM public.theme_regulations tr 
          WHERE tr.theme_id = theme_uuid AND tr.regulation_id = r.id
        );
    END IF;
END $$;